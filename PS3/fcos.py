...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params['conv1x1_c3'] = torch.nn.Conv2d(
            dummy_out_shapes[0][1][1], self.out_channels, 1, stride=1)
        self.fpn_params['conv1x1_c4'] = torch.nn.Conv2d(
            dummy_out_shapes[1][1][1], self.out_channels, 1, stride=1)
        self.fpn_params['conv1x1_c5'] = torch.nn.Conv2d(
            dummy_out_shapes[2][1][1], self.out_channels, 1, stride=1)

        self.fpn_params['conv3x3_p3_1'] = torch.nn.Conv2d(
            self.out_channels, self.out_channels, 3, stride=1, padding=1)
        self.fpn_params['conv3x3_p3_2'] = torch.nn.Conv2d(
            self.out_channels, self.out_channels, 3, stride=1, padding=1)
        self.fpn_params['conv3x3_p4'] = torch.nn.Conv2d(
            self.out_channels, self.out_channels, 3, stride=1, padding=1)
        self.fpn_params['conv3x3_p5'] = torch.nn.Conv2d(
            self.out_channels, self.out_channels, 3, stride=1, padding=1)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        c = [value for _, value in backbone_feats.items()]
        c3, c4, c5 = c
        p3 = self.fpn_params['conv1x1_c3'](c3)
        p4 = self.fpn_params['conv1x1_c4'](c4)
        p5 = self.fpn_params['conv1x1_c5'](c5)
        p4 = p4 + F.interpolate(p5, scale_factor=2)
        p3 = p3 + F.interpolate(p4, scale_factor=2)
        p5 = self.fpn_params['conv3x3_p5'](p5)
        p4 = self.fpn_params['conv3x3_p4'](p4)
        p3 = self.fpn_params['conv3x3_p3_1'](p3)
        p3 = nn.BatchNorm2d(self.out_channels, device=p3.device)(p3)
        p3 = nn.ReLU()(p3)
        p3 = self.fpn_params['conv3x3_p3_2'](p3)
        fpn_feats['p3'] = p3
        fpn_feats['p4'] = p4
        fpn_feats['p5'] = p5
        ######################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        grid = torch.stack(torch.meshgrid(
            torch.arange(w, device=device), torch.arange(h, device=device)), dim=-1)
        grid = grid.reshape(-1, 2)
        xc = (grid[:, 0] + 0.5) * level_stride
        yc = (grid[:, 1] + 0.5) * level_stride

        loc_coords = torch.stack(
            [xc, yc], dim=-1)
        location_coords.update({level_name: loc_coords})
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    kept_indices = torch.ones_like(sorted_indices)
    length = len(sorted_indices)
    for idx0 in range(length-1):
        if (kept_indices[idx0] == 0):
            continue
        box0_area = areas[sorted_indices[idx0]]
        box0 = boxes[sorted_indices[idx0]]
        x10, y10, x20, y20 = box0
        for idx1 in range(idx0+1, length):
            if (kept_indices[idx1] == 0):
                continue
            box1 = boxes[sorted_indices[idx1]]
            x1 = max(x10, box1[0])
            y1 = max(y10, box1[1])
            x2 = min(x20, box1[2])
            y2 = min(y20, box1[3])
            inter_area = max(
                x2-x1, 0) * max(y2 - y1, 0)
            box1_area = areas[sorted_indices[idx1]]
            if ((inter_area / (box0_area + box1_area - inter_area)) > iou_threshold):
                kept_indices[idx1] = 0
    keep = sorted_indices[kept_indices == 1]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return keep
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        stem_cls = []
        stem_box = []
        channel0 = in_channels
        for stem_channel in stem_channels:
            conv_layer1 = torch.nn.Conv2d(
                channel0, stem_channel, 3, stride=1, padding=1)
            nn.init.normal_(conv_layer1.weight, mean=0.0, std=0.01)
            nn.init.zeros_(conv_layer1.bias)
            conv_layer2 = torch.nn.Conv2d(
                channel0, stem_channel, 3, stride=1, padding=1)
            nn.init.normal_(conv_layer2.weight, mean=0.0, std=0.001)
            nn.init.zeros_(conv_layer2.bias)
            stem_cls.append(conv_layer1)
            stem_box.append(conv_layer2)
            stem_cls.append(nn.ReLU())
            stem_box.append(nn.ReLU())
            channel0 = stem_channel
        ...
        self.pred_cls = torch.nn.Conv2d(
            stem_channels[-1], num_classes, 3, stride=1, padding=1)
        self.pred_box = torch.nn.Conv2d(
            stem_channels[-1], 4, 3, stride=1, padding=1)
        self.pred_ctr = torch.nn.Conv2d(
            stem_channels[-1], 1, 3, stride=1, padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
            ...
            c = self.pred_cls(c)
            c = c.permute(0, 2, 3, 1)  # swap height and width dimensions
            c = c.reshape(c.size(0), -1, c.size(-1))  # dynamically determine the number of classes
            class_logits[key] = c
    
            b = self.stem_box(value)
            boxreg = self.pred_box(b)
            boxreg = boxreg.permute(0, 2, 3, 1)  # swap height and width dimensions
            boxreg = boxreg.reshape(boxreg.size(0), -1, 4)
            boxreg_deltas[key] = boxreg
    
            ctr = self.pred_ctr(b)
            ctr = ctr.permute(0, 2, 3, 1)  # swap height and width dimensions
...
def fcos_get_deltas_from_locations(
    ...
    bg_indices = torch.all(gt_boxes == -1.0, dim=1)
    if bg_indices.any():
        bg_deltas = torch.tensor(
            [-1, -1, -1, -1], dtype=deltas.dtype, device=deltas.device)
        bg_deltas = bg_deltas.unsqueeze(0).repeat(bg_indices.sum(), 1)
        deltas[bg_indices] = bg_deltas
...
def fcos_apply_deltas_to_locations(
    ...
    bg_indices = torch.all(deltas == -1.0, dim=1)
    ...
    if bg_indices.any():
        output_boxes[bg_indices] = locations.repeat(1, 2)[bg_indices]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = torch.sqrt(
        (torch.min(torch.stack([left, right]), dim=0).values * torch.min(torch.stack([top, bottom]), dim=0).values) /
        (torch.max(torch.stack([left, right]), dim=0).values *
         torch.max(torch.stack([top, bottom]), dim=0).values)
    )

    centerness[deltas.min(dim=1)[0] == -1] = -1
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Feel free to delete this line: (but keep variable names same)
        shape_per_fpn_level = {}
        shape_per_fpn_level['p3'] = fpn_feats['p3'].shape
        shape_per_fpn_level['p4'] = fpn_feats['p4'].shape
        shape_per_fpn_level['p5'] = fpn_feats['p5'].shape
        ...
        for matched_boxes in matched_gt_boxes:
            ...
            for level in locations_per_fpn_level.keys():
                locations = locations_per_fpn_level[level]
                matched_boxes_this_level = matched_boxes[level]
                gt_deltas[level] = fcos_get_deltas_from_locations(
                    locations, matched_boxes_this_level, self.backbone.fpn_strides[level])
        ...
        annotated_cls = matched_gt_boxes[:, :, -1].to(torch.int64)
        targets = torch.zeros_like(pred_cls_logits)
        targets[annotated_cls >= 0] = F.one_hot(
            annotated_cls[annotated_cls >= 0], pred_cls_logits.shape[-1]).to(targets.dtype)
        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=targets)
        loss_box = 0.25 * \
            F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none")
        loss_box[annotated_cls < 0] *= 0.0
        matched_gt_centerness = fcos_make_centerness_targets(
            deltas=torch.reshape(matched_gt_deltas, (-1, 4)))
        matched_gt_centerness = matched_gt_centerness.view(
            pred_ctr_logits.shape)
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits, matched_gt_centerness, reduction="none")
        loss_ctr[annotated_cls < 0] *= 0.0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            _, level_pred_classes = torch.max(
                level_cls_logits.sigmoid_(), dim=1)
            ...
            level_pred_classes = level_pred_classes[indices]
            ...
            level_pred_boxes[:, 0] = torch.clamp(
                level_pred_boxes[:, 0], 0, width)
            level_pred_boxes[:, 1] = torch.clamp(
                level_pred_boxes[:, 1], 0, height)
            level_pred_boxes[:, 2] = torch.clamp(
                level_pred_boxes[:, 2], 0, width)
            level_pred_boxes[:, 3] = torch.clamp(
                level_pred_boxes[:, 3], 0, height)
