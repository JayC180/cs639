...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.zeros((N, F, H_output, W_output), dtype = x.dtype, device=x.device)
        
        for n in range(N):
          for f in range(F):
            for h_output in range(H_output):
              for w_output in range(W_output):
                x_window = x_padded[n, :, h_output*stride:h_output*stride+HH, w_output*stride:w_output*stride+WW]
                result = torch.sum(x_window * w[f])
                out[n, f, h_output, w_output] = result + b[f]
    ...
    def backward(dout, cache):
      ...
      H_output = int(1 + (H + 2 * pad - HH) / stride)
      W_output = int(1 + (W + 2 * pad - WW) / stride)
      _, _, H_output, W_output = dout.shape
      ...
      for n in range(N):
        for f in range(F):
          ...
          for h_output in range(H_output):
            for w_output in range(W_output):
              dx_padded[n, :, h_output*stride:h_output*stride+HH, w_output*stride:w_output*stride+WW] += w[f] * dout[n, f, h_output, w_output]
              dw[f] += x_padded[n, :, h_output*stride:h_output*stride+HH, w_output*stride:w_output*stride+WW] * dout[n, f, h_output, w_output]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        H_output = 1 + (H - pool_height)
        W_output = 1 + (W - pool_width) 
        out = torch.zeros((N, C, H_output, W_output), dtype = x.dtype, device=x.device)
          ...
          for c in range(C):
            for h in range(H_output):
              for w in range(W_output):
                ...
                if window.numel() == 0:
                  continue
                out[n, :, h, w] = torch.max(window.reshape((C, -1)), dim=1, keepdim=True)[0]
    ...
    def backward(dout, cache):
        ...
        for n in range(N):
          for c in range(C):
            for h in range(H_output):
                for w in range(W_output):
                  index = torch.argmax(x[n, c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width])
                  ind = (index // pool_width, index % pool_width)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.params['W1'] = torch.randn(num_filters, input_dims[0], filter_size, filter_size) * weight_scale
        W2_row_size = int(num_filters * input_dims[1]/2 * input_dims[2]/2)
        self.params['W2'] = torch.randn(W2_row_size, hidden_dim) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out1, cache1 = Conv_ReLU_Pool.forward(X, self.params['W1'].cuda(), self.params['b1'].cuda(), conv_param, pool_param)
        out2, cache2 = Linear_ReLU.forward(out1, self.params['W2'].to(out1.device), self.params['b2'].to(out1.device))
        out3, cache3 = Linear.forward(out2, self.params['W3'].to(out2.device), self.params['b3'].to(out2.device))
        scores = out3
        ...
        reg_loss = 0.5 * self.reg * (torch.sum(W1 ** 2) + torch.sum(W2 ** 2) + torch.sum(W3 ** 2))
        ...
        dW3 += self.reg * W3.to('cuda')
        dW2 += self.reg * W2.to('cuda')
        dW1 += self.reg * W1.to('cuda')
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers):
            F_in = C if i == 0 else num_filters[i-1]
            F_out = num_filters[i] if i < len(num_filters) else num_classes
            self.params[f'W{i+1}'] = nn.Parameter(torch.randn(F_out, F_in, 3, 3) * weight_scale)
            self.params[f'b{i+1}'] = nn.Parameter(torch.zeros(F_out))
            if self.batchnorm and i < self.num_layers-1:
                self.params[f'gamma{i+1}'] = nn.Parameter(torch.ones(F_out))
                self.params[f'beta{i+1}'] = nn.Parameter
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.rand((Din, Dout))
        weight = weight * sqrt(gain/Din)
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.rand((Din, Dout, K, K))
        weight = weight * sqrt(gain/Din)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
