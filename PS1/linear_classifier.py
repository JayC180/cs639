...
def sample_batch(
    ...
    i = torch.randint(num_train, size=(batch_size,))
    X_batch = X[i]
    y_batch = y[i]
...
def softmax_loss_naive(
    ...
    # xw = [128, 3073] x [3073, 10] -> [128, 10]
    # y = 128

    # number of things in X
    t = X.shape[0]
    # classes
    c = W.shape[1]
    ...
    for i in range(t):
      # score
      s = torch.matmul(X[i], W)
      # 128
      s -= torch.max(s)
      correct_s = s[y[i]]

      # ezi/sum ezj
      ezj = torch.sum(torch.exp(s))
      loss += -torch.log(torch.exp(correct_s) / ezj)

      for j in range(c):
        if j == y[i]:
          dW[:, y[i]] += X[i, :] * (torch.exp(s[j]) / ezj - 1)
        else:
          dW[:, j] += X[i, :] * (torch.exp(s[j]) / ezj)
...
def softmax_loss_vectorized(
    ...
    s = torch.matmul(X, W)
    s = s - torch.max(s, dim=1)[0][:, None]
    
    loss = -torch.sum(torch.log(torch.exp(s[torch.arange(t), y]) / torch.sum(torch.exp(s), dim=1)))
    loss /= t
    loss += 0.5 * reg * torch.sum(W * W)

    i = torch.zeros_like(s)
    i[torch.arange(t), y] = 1

    dW = torch.matmul(X.T, torch.exp(s) / torch.sum(torch.exp(s), dim=1, keepdims=True) - i)
    dW /= t
    dW += reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
